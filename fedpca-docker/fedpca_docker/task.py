...
def load_autoencoder_model(input_size: int, encoded_size: int): 
    model = keras.Sequential(
        [
            ...
            layers.Dense(encoded_size, activation='relu', name = 'bn'),
...
def load_data(tipo_dados, pre_processamento, oversample):
    if tipo_dados == 'fedpca' and pre_processamento == False:
       if oversample == True:
          ...
          if os.path.exists("/app/x_train_pca_resampled.csv"): 
             ...
             y_train = pd.read_csv("/mnt/fl_conj_treino_cliente.csv")['subtipo2']
             y_test = pd.read_csv("/mnt/fl_conj_valid_cliente.csv")['subtipo2']
    elif tipo_dados == 'autoencoder' and pre_processamento == False:
       if oversample == True:
